...
urlpatterns = [
    path('register/', RegisterView.as_view(), name='register'),  # POST endpoint for user registration
    path('login/', LoginView.as_view(), name='login'),           # POST endpoint for user login
    path('profile/', ProfileView.as_view(), name='profile'),     # GET endpoint to fetch logged-in user's profile
]
